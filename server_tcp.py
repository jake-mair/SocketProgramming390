...
def keywordTask(conn):
    conn.sendall(
        b"Keyword command received."
    )  # Acknowledge that the command was received
    ...
    ext = parts[1]
    anonymized_filename = (
        f"{name}_anon.{ext}"  # Create a new filename for the anonymized file
    )
